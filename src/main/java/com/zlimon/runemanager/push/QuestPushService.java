...
/**
 * Builds and pushes the player's full quest snapshot. Driven by
 * {@link AccountSnapshotService} (once per login and whenever the token is set),
 * so a freshly linked account uploads its quests without needing a completion
 * event. The server treats the payload as a snapshot upsert, so re-running is
 * harmless.
 */
@Slf4j
@Singleton
